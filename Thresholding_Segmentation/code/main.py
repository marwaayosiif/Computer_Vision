...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def Kmeans(self):
        ...
        grayKmeansOutput = Kmeans.grayKmeans().Kmeans_Gray(self.grayImage)
    ...
    def LocalThresholding(self, Image, flag):
        ...
        for i in range(self.N):
            for j in range(self.N):
                mask = orgImage[i * hR:hR * (i + 1), j * hC:hC * (j + 1)]
    ...
    def ostu(self):
        ...
        inputImg = self.grayThImage
        if status == 'Global Thresholding':
            Th = self.Th_functions[1](inputImg)
            self.GlobalThresholding(inputImg,Th)

        elif status == "Local Thresholding":
            self.LocalThresholding(inputImg, 1)
        
        elif status == "Spectral":
            self.spectral(inputImg)

    def spectral(self,grayImage):
        Th = self.Th_functions[1](grayImage)
        blur = grayImage
        mask = blur >Th
